...
export class MarketFilterPage {
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad MarketFilterPage');
  }
  ionViewWillLeave() {
    this.Filter_Data = {MarketType: this.MarketType };
    this.events.publish('Fil_Market_Data', this.Filter_Data);

  }
  Set_Fil() {
    this.events.subscribe('Set_Filter_Market', (Set_Data) => {
      ...
      // this.Load_Data();
      this.events.unsubscribe('Set_Filter_Market');
    });
  }
  Get_Markett_Type() {
